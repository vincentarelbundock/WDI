...
To speed up search and provide country metadata, `WDI` ships with a
local cache of available WDI series and countries. This cache is updated
semi-regularly, but you may still want to refresh it manually to access
the latest metadata. To do so, use `WDIcache()` and pass its result to
`WDIsearch()` or `WDI()`:

``` r
new_cache <- WDIcache()
WDIsearch("gdp", cache = new_cache)
WDI(country = "US", indicator = "NY.GDP.PCAP.KD", extra = TRUE,
    cache = new_cache)
```
